...
| 运行名 | 词表大小 | 参数量 | 最佳验证损失 | 最佳验证困惑度 | 关键意义 |
| --- | ---: | ---: | ---: | ---: | --- |
| `tinyshakespeare-subword-gpt-v1` | 512 | 6,490,624 | `2.9711` | `19.51` | 补齐 tokenizer、special tokens、padding mask 和采样控制 |
...
`subword-gpt v1` 在 `temperature = 0.6` 下的样例摘录：
...
`char-transformer v3` 和 `subword-gpt v1` 的 token 粒度不同，因此它们的困惑度不适合直接当作绝对优劣结论。

更准确的理解方式是：

- `char-transformer v3` 更适合展示最小语言模型和自注意力主线已经真正跑通。
- `subword-gpt v1` 更适合展示工程流程已经从教学版模型推进到更接近真实 GPT 的形态。
