...
你不需要先决定“按空格切词”还是“按字符切词”，而是直接从 byte 出发，再让 merge 规则自己长出来。
...
很多人第一次写语言模型时，会把样本 pad 到同样长度，但忽略了后续两件事：
...
如果你照常把这些 `<pad>` 也送进交叉熵，模型就会被迫学习“如何预测 pad”，这会污染训练目标。
...
如果把这一步看成“从教学版 Transformer 到更像真实 GPT 的过渡”，核心升级其实有五个：
...
但如果要继续往前走，下一步最值得补的并不是再横向学一个完全不同的模型，而是把语言模型这条线做得更完整：
