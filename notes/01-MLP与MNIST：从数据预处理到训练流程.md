...
这篇笔记围绕一个典型的入门任务展开：使用多层感知机（MLP）完成 MNIST 手写数字识别。重点不在于追求复杂模型，而在于梳理一个最基础的图像分类流程（pipeline）是如何搭起来的，包括数据预处理、模型结构、损失函数、优化器和训练循环。
...
> ![分段线性示意图](../assets/images/01-mlp-mnist/de2ca7bc7b1f489f8834f2e151b2fcc8.png)
...
可视化效果如图所示：
> ![分段函数可视化示意图](../assets/images/01-mlp-mnist/b05cbe5bd0d44591ad6999db3dc8c17b.png)
...
\end{cases}
$$
这里的 $b_i$ 和 $w_i$ 也都已经是重参数化后的量。这样的分段函数可视为硬 Sigmoid（Hard Sigmoid）的一种直观原型。整体预测模型可以表示为：
$$
y=\mathrm{const}+\sum_i \mathrm{HardSigmoid}_i(x)
$$
它的意义在于说明：只要把多个带阈值的非线性单元叠加起来，就可以表示更复杂的函数形状。这也为后续引入连续可导的激活函数做了铺垫。

### 用 Sigmoid 函数构造可导的非线性单元

硬 Sigmoid（Hard Sigmoid）的直觉比较清晰，但分段形式并不够平滑。更常见的替代是连续可导的 Sigmoid 函数：
$$
\sigma(x) = \frac{1}{1+e^{-x}}
$$
它的 S 形曲线与“阈值逐渐开启”的行为比较接近：
> ![Sigmoid 曲线示意图](../assets/images/01-mlp-mnist/d730d42c4ad34214843afda68f58cc14.png)
...
下面结合结构图看一次前向计算过程：
> ![神经网络前向传播示意图](../assets/images/01-mlp-mnist/90dc1fbe58d4462eb8c7fc0c75c21d91.png)
...
> ![神经元激活示意图](../assets/images/01-mlp-mnist/2ffba6ed39f74e019eb50dea9fb0a467.png)
...
具体结构如下图所示：
> ![MLP 结构示意图](../assets/images/01-mlp-mnist/ab3150a8e7bb448ba31ecb1b652b8b49.png)
...
> ![损失曲面下降示意图](../assets/images/01-mlp-mnist/925e7a260e7f4916b6da747d4cadbd7e.png)
...
> ![梯度方向示意图](../assets/images/01-mlp-mnist/df4db9b06da7434281306d533d0cfee4.png)
...
![MNIST 预测可视化结果](../assets/images/01-mlp-mnist/24eca9e6098240f28db8e2a8c8303d78.png)
